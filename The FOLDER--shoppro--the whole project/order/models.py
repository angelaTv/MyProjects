...
from product.utils import unique_order_id_generator
...
def post_save_cart_total(sender, instance, created, *args, **kwargs):
    if not created:
        ...
        cart_id = cart_obj.id
        qs = Order.objects.filter(cart__id=cart_id)  # the order should be one per cart
        if qs.count() == 1:  # the order should be one per cart
            order_obj = qs.first()
